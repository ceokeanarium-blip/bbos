...
// Hàm tạo số phiên bản Chrome ngẫu nhiên trong phạm vi phiên bản mới
function getRandomChromeVersion() {
    // Phiên bản chính (major version) - giữ trong phạm vi 120-140 để có phiên bản mới
    const majorVersion = Math.floor(Math.random() * 20) + 120;
    // Phiên bản phụ (minor version)
    const minorVersion = Math.floor(Math.random() * 9000) + 1000;
    // Phiên bản sửa lỗi (patch version)
    const patchVersion = Math.floor(Math.random() * 300);
    
    return `${majorVersion}.0.${minorVersion}.${patchVersion}`;
}

// Tạo mảng user agent cơ bản cho Google bot
const googleBotBaseTemplates = [
    'Mozilla/5.0 (Linux; Android 7.0; SM-G930V Build/NRD90M) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/VERSION Mobile Safari/537.36 (compatible; Google-Read-Aloud; +https://support.google.com/webmasters/answer/1061943)',
    'Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; Googlebot/2.1; +http://www.google.com/bot.html) Chrome/VERSION Safari/537.36',
    'Mozilla/5.0 (Linux; Android 6.0.1; Nexus 5X Build/MMB29P) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/VERSION Mobile Safari/537.36 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)',
    'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)',
    'Mozilla/5.0 (Linux; Android 7.0; SM-G930V Build/NRD90M) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/VERSION Mobile Safari/537.36 (compatible; Googlebot-Mobile; +http://www.google.com/bot.html)',
    'Mozilla/5.0 (compatible; Mediapartners-Google/2.1; +http://www.google.com/bot.html)',
    'Mozilla/5.0 (compatible; AdsBot-Google/3.1; +http://www.google.com/bot.html)',
    'Mozilla/5.0 (compatible; Google-PageRenderer/1.0; +http://www.google.com/bot.html)',
    'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/VERSION Mobile Safari/537.36',
    'Mozilla/5.0 (compatible; Googlebot-Mobile/2.1; +http://www.google.com/bot.html)'
];

// Hàm tạo user agent Google bot với phiên bản Chrome ngẫu nhiên
function generateGoogleBotUserAgent() {
    // Chọn ngẫu nhiên một mẫu user agent từ danh sách
    const template = googleBotBaseTemplates[Math.floor(Math.random() * googleBotBaseTemplates.length)];
    
    // Nếu mẫu có chứa "VERSION", thay thế bằng phiên bản Chrome ngẫu nhiên
    if (template.includes('VERSION')) {
        return template.replace('VERSION', getRandomChromeVersion());
    }
    
    // Trả về nguyên mẫu nếu không có "VERSION" (một số bot Google không có phiên bản Chrome)
    return template;
}

// Tạo mảng user agent với 10 user agent ngẫu nhiên
function generateGoogleBotUserAgents(count = 10) {
    const userAgents = [];
    for (let i = 0; i < count; i++) {
        userAgents.push(generateGoogleBotUserAgent());
    }
    return userAgents;
}

// Khởi tạo mảng uap với user agent Google bot và phiên bản Chrome ngẫu nhiên
const uap = generateGoogleBotUserAgents();

// Thêm hàm để tự động làm mới danh sách user agent sau mỗi khoảng thời gian
setInterval(() => {
    // Cập nhật lại mảng uap với các user agent mới
    const newUserAgents = generateGoogleBotUserAgents();
    uap.length = 0; // Xóa tất cả các phần tử hiện tại
    
    // Thêm các user agent mới vào mảng
    newUserAgents.forEach(ua => uap.push(ua));
    
    console.log("Đã cập nhật danh sách user agent Google bot");
